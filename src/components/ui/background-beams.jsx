...
const generateGradientConfigs = (count) => {
...
export function BackgroundBeams({ className }) {
  ...
  const gradientConfigs = useMemo(() => generateGradientConfigs(paths.length), [paths.length]);
          ...
          {paths.map((_path, index) => {
            ...
            return (
              <linearGradient
                ...
                y2={config.y2}
              >
                <stop stopColor="#FF2D2D" stopOpacity="0" />
                <stop stopColor="#FF2D2D" />
                <stop offset="32.5%" stopColor="#00FFD1" />
